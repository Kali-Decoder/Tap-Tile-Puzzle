...
const Game = ({ rows = 5, cols = 5 }) => {
  const { addTransaction, loading } = useDataContext();
  ...
  return (
    <div className=" w-[90%] flex flex-col justify-center items-center h-[100%] gap-y-4.5 text-2xl">
      ...
      <code className="text-red-500">{loading ? "Loading..." : null}</code>
      <p>Moves: {moves}</p>
          ...
          <div key={rIdx} className="row">
            {row.map((cell, cIdx) => (
              <div
                ...
                onClick={async () => {
                  let status = await handleTileClick();
      ...
      {isSolved && (
        <p className="win-message">You solved it in {moves} moves! 🎉</p>
      )}
    </div>
  );
};
